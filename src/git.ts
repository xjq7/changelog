...
export async function generateTag(name: string) {
  return execa('git', ['tag', '-a', name, '-m', 'tubitlog 自动创建']).then(
    res => res.stdout
  );
}
...
export async function deleteRemoteTag(name: string) {
  return execa('git', ['push', 'origin', '--delete', name]).then(
    res => res.stdout
  );
}

export async function getFileContentByCommitId(commitId: string, file: string) {
  return execa('git', ['cat-file', '-p', `${commitId}:${file}`]).then(
    res => res.stdout
  );
}
